...
@endif
    ...
    <script src="{{ Knuckles\Scribe\Tools\WritingUtils::getVersionedAsset($assetPathPrefix.'js/theme-default.js') }}"></script>
...
<script>
let authScribe = localStorage.getItem('auth-scribe');
let authInput = document.querySelectorAll('[name=Authorization]');

function handleAuthInput(ev){
  localStorage.setItem('auth-scribe',ev.target.value);
}
authInput.forEach(el=>{
  el.addEventListener('input', handleAuthInput)
  if(authScribe){
    el.value=authScribe
  }
})
